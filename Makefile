...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://aurora-training-project.localhost"
...
acceptance-backend-image-build:
	...
	@docker build backend -t collective/aurora-training-project-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name aurora-training-project-frontend-acceptance --link aurora-training-project-backend-acceptance:backend -e PLONE_API_PATH=http://backend:55001/plone -d collective/aurora-training-project-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name aurora-training-project-backend-acceptance -d collective/aurora-training-project-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop aurora-training-project-frontend-acceptance
	@docker stop aurora-training-project-backend-acceptance
...
acceptance-playwright-test: ## Start Playwright acceptance tests in interactive (UI) mode (against already-running acceptance servers)
