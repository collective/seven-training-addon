...
DEPLOY_ENV=prod
DEPLOY_HOST=aurora-training-project.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for aurora-training-project.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    aurora-training-project.example.com:
      ansible_user: root
      host: aurora-training-project
      hostname: aurora-training-project.example.com
```
...
Deploy the stack defined in `devops/stacks/aurora-training-project.example.com.yml` to the remote server with:
