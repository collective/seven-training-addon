...
2. Go to your repository at [collective/aurora-training-project](https://github.com/collective/aurora-training-project).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `aurora-training-project.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | aurora-training-project.example.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | aurora-training-project.example.com |
...
- [Backend Tests Workflow](https://github.com/collective/aurora-training-project/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/aurora-training-project/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/collective/aurora-training-project-backend`) and Frontend (`ghcr.io/collective/aurora-training-project-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of aurora-training-project.example.com](https://github.com/collective/aurora-training-project/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/aurora-training-project.example.com.yml` stack.
